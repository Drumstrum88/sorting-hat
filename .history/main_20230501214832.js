const students = [
  ...
  const slyBtn = document.querySelector('#slytherin');
   ...
   const stupify = document.querySelector('#expel');

   stupify.addEventListener('click', (e) =>
   {
    if (e.target.id.includes("#expel")) {
      const [, id] = e.target.id.split("--");

      
      const index = students.findIndex(student => e.id === Number(id));
      const expelledStudent = students.splice("--")
